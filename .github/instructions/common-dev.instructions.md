...
- Never commit directly to main branch - only via PR from dev branch with owner approval
- Always check .gitignore before adding files and never commit secrets or sensitive data
- Large files (ISOs, images) are downloaded on-demand, not stored in repository
